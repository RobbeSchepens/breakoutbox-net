...
namespace BreakOutBox.Data.Repositories
{
    public class LeerkrachtRepository : ILeerkrachtRepository
    {
        ...
        {
            return _leerkrachten.Where(lk => lk.Email == email)
                .Include(lk => lk.Sessies).ThenInclude(ses => ses.Groepen)
                .Include(lk => lk.Sessies).ThenInclude(ses => ses.Klas)
                .FirstOrDefault();
        }

        public Leerkracht GetByVolledigeNaam(string voornaam, string achternaam)
        {
            return _leerkrachten.Where(lk => lk.Voornaam == voornaam && lk.Achternaam == achternaam)
                .Include(lk => lk.Sessies).ThenInclude(ses => ses.Groepen)
                .Include(lk => lk.Sessies).ThenInclude(ses => ses.Klas)
                .FirstOrDefault();
        }
