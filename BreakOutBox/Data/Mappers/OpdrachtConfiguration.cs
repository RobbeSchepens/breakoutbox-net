...
namespace BreakOutBox.Data.Mappers
{
    public class OpdrachtConfiguration : IEntityTypeConfiguration<Opdracht>
    {
        public void Configure(EntityTypeBuilder<Opdracht> builder)
        {
            ...
            builder.HasOne(s => s.Groepsbewerking).WithOne(g => g.Opdracht).IsRequired().OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(s => s.Toegangscode).WithOne(s => s.Opdracht).IsRequired().OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(s => s.Actie).WithOne(s => s.Opdracht).IsRequired().OnDelete(DeleteBehavior.Cascade);
        }
    }
}
